...
실제 가입 계약이 없는 사용자의 보험 선택 화면에는 예시 보험 3개를 `데모용`으로 표시합니다. 이 경로와 독립 fixture 미리보기에서는 신청·결과 화면도 `데모`로 구분하며, 실제 보험사 제출·할인 결정·증명 검증을 했다고 표시하지 않습니다. 별도 로컬 연동 데모에서도 증명 및 체인 결과가 없으면 `검증 정보 없음` 또는 `연결 전`으로 표시하고 임의의 성공 상태나 해시를 보여주지 않습니다. 화면 문구 변경은 연동 범위를 넓히지 않습니다.

2026-09-27 화면 체험 모드에서 가입 화면 → 예시 보험 선택 → 두 번의 모의 운행 → 데모 신청·결과 → 홈 복귀를 브라우저로 확인했습니다. 주행 결과에서 홈 복귀가 주행 탭으로 향하던 이동 경합을 수정했습니다. 실제 Privy·DB·C/Wallet·Midnight 종단 및 iOS/Android 실기기 검증은 포함되지 않습니다.
