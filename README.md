...
`apps/mobile` is a deterministic subscriber-facing Expo Router demo. It covers onboarding consent, selecting one of three local example policies, and a three-tab flow: **Home**, **Driving**, and **Documents**. The consent sheet distinguishes locally processed demo data, the insurer-facing summary, and raw driving details that are not provided. The primary navigation uses a floating iOS-style pill, and policy selection uses a card-border-only selected state with insurer, policy status, vehicle number, and coverage period. Two simulated trips deterministically progress from 0 km / 100 points through 300 km / 92 points to 550 km / 87 points and a 10% expected discount. The application review expressly excludes precise location, route, segment speed, and exact driving time.
...
Verification on 2026-09-22: 48 mobile Jest tests, including hydrated consent/policy deep-route protection, consent-detail disclosure, exactly-once authorized simulated-trip processing, the Documents tab route, and policy identifying metadata; mobile typecheck/lint, Expo Doctor (21/21), and Expo web/iOS/Android export passed. The web flow was manually checked at 402×874 and 360×740 with consent, insurance, two drives, review, pending demo approval, and result. The updated consent sheet, pill navigation, and policy-card selected state were additionally checked at 402×874. This is browser QA only; physical iOS/Android devices and Expo Go have not been exercised.
