...
`20260921190000_add_initial_registration_attempts.sql`은 최초 배포 외부 호출 전 `operation_id`를 Scope·runtime별로 영속화하는 후속 migration이며 **Supabase 원격에 적용 완료됐다**. B의 Fake Adapter 검사는 같은 ID의 C `not-submitted`/`chain-confirmed` 상태에 따라 안전한 재시도·DB 복구를 분기한다. 실제 C의 영속 거래 상태 조회를 제공하는 운영 `BCAdapter`는 아직 없으므로, 원격 테이블 적용과 운영 복구 완성을 구분한다. 기존 원격 프로젝트에 이 migration을 다시 실행하지 않는다. 앞서 추가된 `20260918060000_chain_state_confirmation.sql`은 별도 로컬 검증용이며 원격 적용 완료로 보지 않는다.
