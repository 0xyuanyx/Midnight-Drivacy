...
`apps/mobile/src/api/backend.ts`의 `createDriverApi({ baseUrl, getAccessToken })`는 Auth 확인, 동의, 보험계약·특약 목록/선택, 모의 운행 시작·조회·종료·처리 접수·상태 조회 요청을 캡슐화한다. 동의 시간·사용자 ID, Rule/State/점수는 클라이언트 요청에 넣지 않는다. 운행 시작과 처리 접수의 멱등 키는 요청 본문이 아니라 `Idempotency-Key` 헤더로 보낸다. 토큰이 없으면 네트워크 요청 전에 `AUTH_REQUIRED` 오류를 낸다. 상태 조회에서 Backend의 `db-confirmed`와 공개 요약을 받더라도 기존 화면이 이를 자동 표시하는 단계는 아니다.
