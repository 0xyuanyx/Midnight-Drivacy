...
Each subscriber scope has its own Chain Contract. `BCAdapter.deployRule({ operationId, scope, approvedRule })` is used only when that scope has no deployment; B persists the operation ID before invoking C. It returns a `DeployRuleResult` with the same operation ID, deployment transaction ID, confirmed registered rule, and chain-confirmed Genesis State after `initialize`. B binds that result to the reserved operation, approved Rule, Scope, runtime, and contract, then stores deployment, registration, current Rule, and Genesis in one DB transaction. A deployment receipt alone does not activate the Rule. `BCAdapter.updateRule({ scope, deployment, approvedRule })` updates a later approved Rule version on the existing contract.

이것은 B↔C 응답 계약과 B의 Fake Adapter 검사다. 동일 Scope/runtime에 미해결 최초 등록 시도가 남으면 다른 Version 요청은 배포를 다시 부르지 않는다. 같은 Version은 `getInitialRegistrationStatus(operationId)`의 신뢰된 결과가 `not-submitted`일 때만 같은 ID로 재시도하고, `chain-confirmed`라면 배포 없이 기존 결과를 DB에 기록한다. `pending`이나 조회 실패에서는 멈춘다. `not-submitted`는 C의 영속 거래 기록과 체인 조회로 제출 부재를 확인한 경우여야 하며 단순히 로컬 journal에 값이 없다는 뜻이 아니다. C의 운영용 어댑터·가입자 브라우저 승인 연결·실제 상태 조회 구현은 아직 없으므로 운영 등록 API는 활성화하지 않는다.
...
The registration service receives an `AdapterRuntime` (`network`, `adapterProfile`) and selects deployments by that complete identity. Deployments for other runtimes may coexist for the same Scope and are neither ambiguous nor eligible for reuse. Before an external adapter call, B takes a PostgreSQL session advisory lock keyed by evaluation scope and adapter runtime; registration queries and writes reuse that connection, while the attempt reservation commits before the external call. A Rule-version key would permit concurrent first registrations of different versions to deploy two contracts; the database unique constraint would reject the later insert only after a duplicate chain side effect.
