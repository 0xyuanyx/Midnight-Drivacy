...
export function Riders() {
  ...
  return <>
    ...
    <div className="workspace-grid">
      ...
      <section className="panel detail-panel" aria-label="특약 상세">
        ...
        <div className="detail-tabs" role="tablist" aria-label="특약 상세 탭">{riderTabs.map(([id, label]) => <button key={id} type="button" role="tab" aria-selected={tab === id} onClick={() => setTab(id)}>{label}</button>)}</div>
        <div className="detail-content" role="tabpanel">
          ...
          {tab === "conversion" ? <div className="operations-stack"><div className="operations-step-nav" role="tablist" aria-label="규칙 변경 단계"><button type="button" aria-selected={stage === "upload"} onClick={() => setStage("upload")}>1. 문서 업로드</button><button type="button" aria-selected={stage === "edit"} disabled={!documentName} onClick={() => setStage("edit")}>2. 내용 확인·수정</button><button type="button" aria-selected={stage === "draft"} disabled={!policyText.trim()} onClick={() => setStage("draft")}>3. 규칙 초안</button><button type="button" aria-selected={stage === "manual"} onClick={() => setStage("manual")}>수기 보완</button></div>
            {stage === "upload" ? <div className="operations-card"><div className="operations-card-head">특약 문서 업로드</div><div className="operations-card-body"><label className="operations-upload"><strong>변경할 특약 문서를 선택하세요</strong><span>PDF, DOCX, TXT, MD · 최대 10MB</span><input aria-label="특약 문서 업로드" type="file" accept=".pdf,.docx,.txt,.md" disabled={isReading} onChange={(event) => { void uploadDocument(event.target.files?.[0]); event.target.value = ""; }} /></label>{isReading ? <p className="operations-copy" role="status">문서 내용을 읽고 있습니다.</p> : null}{documentError ? <p className="operations-error" role="alert">{documentError}</p> : null}{documentName ? <p className="operations-copy">현재 문서: {documentName} · <button className="operations-link" type="button" onClick={() => setStage("edit")}>내용 확인·수정</button></p> : null}</div></div> : null}
        ...
        <div className="operations-panel-foot"><span>변환 결과의 저장·승인·적용 기능은 서비스 연결 후 제공됩니다.</span><button className="button primary" type="button" onClick={() => { setTab("conversion"); setStage("upload"); }}>규칙 변경</button></div>
      </section>
    </div>
  </>;
}
...
export function VerificationHistory({ state, onOpenEvaluation }: { state: WorkspaceState; onOpenEvaluation: (id: string) => void }) {
  ...
  return <><p className="operations-note operations-top-note">검증 결과가 연결되면 증명 상태와 온체인 거래 정보를 확인할 수 있습니다.</p><div className="workspace-grid"><section className="panel" aria-label="검증 이력 목록"><div className="panel-heading"><div><h2>검증 이력</h2><span>{items.length}건</span></div></div><div className="list-tools"><label className="search-field"><input aria-label="신청번호 검색" placeholder="신청번호 검색" value={query} onChange={(event) => setQuery(event.target.value)} /></label><select aria-label="검증 상태 필터" disabled><option>검증 정보 없음</option></select></div><div className="operations-list-head"><span>신청번호 / 특약</span><span>검증</span></div>{items.length === 0 ? <div className="empty-state">검색 결과가 없습니다.</div> : items.map((item) => <button className={`operations-list-row ${selected?.id === item.id ? "selected" : ""}`} key={item.id} type="button" onClick={() => setSelectedId(item.id)}><span><strong>{item.displayId}</strong><small>{item.riderName}</small></span><span>{"검증 정보 없음"}</span></button>)}</section>
    <section className="panel detail-panel" aria-label="선택한 검증 이력">{selected ? <><div className="detail-heading"><div><span className="detail-eyebrow">검증 기록</span><h2>{selected.displayId}</h2><p>실제 체인 거래와 검증 영수증은 연결 전입니다.</p></div><span className="status-badge">체인 연결 전</span></div><div className="detail-content operations-stack"><div className="operations-card"><div className="operations-card-head">검증 출처</div><Row label="화면 데이터" value="조회 전" /><Row label="실제 ZK 검증" value="확인할 수 없음" /><Row label="온체인 거래" value="연결 전" /></div><p className="operations-note">점수·규칙·증명 상세와 처리 이력은 평가 요청 상세에서 확인합니다. 증명 검증 정보는 아직 연결되지 않았습니다.</p></div><div className="operations-panel-foot"><span>선택한 신청의 상세로 이동합니다.</span><button className="button secondary" type="button" onClick={() => onOpenEvaluation(selected.id)}>평가 요청 상세로 이동</button></div></> : <div className="empty-state">선택할 기록이 없습니다.</div>}</section></div></>;
}

export function Connections() {
  const rows = [["Backend API", "서버 응답과 업무 기능 연결", "확인 전"], ["가입자 앱 데이터", "가입자 신청 전달", "미연동"], ["규칙 처리 서비스", "자동 변환·승인·등록 흐름", "확인 전"], ["보험사 업무 시스템", "할인 적용 결과 반영", "미연동"]];
  return <><section className="panel operations-connections" aria-label="연동 상태"><div className="panel-heading"><div><h2>연동 상태</h2><span>읽기 전용</span></div></div>{rows.map(([name, description, status]) => <div className="operations-connection" key={name}><strong>{name}</strong><small>{description}</small><span className="status-badge">{status}</span></div>)}</section><p className="operations-note operations-top-note">서비스 연결 상태는 아직 조회되지 않습니다. 이 화면에서 변경할 설정은 없습니다.</p></>;
}
