...
import { Check, CircleAlert, FileCheck2, Search, X } from "lucide-react";
...
import { readLocation, toUrl, type DetailTab, type LocationState, type Workflow } from "./navigation";

const tabsByWorkflow: Record<Workflow, Array<{ id: DetailTab; label: string }>> = {
...
function SideNav({ active, onNavigate, onUtility }: { active: Workflow; onNavigate: (workflow: Workflow) => void; onUtility: (name: string) => void }) {
  return <aside className="sidebar">
    ...
    <nav className="primary-nav" aria-label="주요 메뉴">
      ...
      {["특약 관리", "규칙 변환", "검증 이력", "연동 설정"].map((name) => <button key={name} type="button" onClick={() => onUtility(name)}><i />{name}</button>)}
    </nav>
...
function PageHeader({ workflow, onUtility }: { workflow: Workflow; onUtility: (name: string) => void }) {
  ...
  return <header className="page-header">
    <div><h1>{dashboard ? "대시보드" : "평가 요청"}</h1><p>{dashboard ? "오늘 처리할 특약 승인 요청과 안전운전 평가를 확인합니다." : "안전운전 할인특약 평가 결과를 검증하고 처리합니다."}</p></div>
    <div className="header-actions"><button aria-label="도움말" className="icon-button" type="button" onClick={() => onUtility("도움말")}>?</button><button aria-label="알림" className="icon-button" type="button" onClick={() => onUtility("알림")}>♧</button><button className="profile-button" aria-label="계정" onClick={() => onUtility("계정")}>김</button></div>
  </header>;
}

function SummaryGrid({ workflow, state }: { workflow: Workflow; state: WorkspaceState }) {
...
function RequestList({ workflow, state, selectedId, query, filter, onSelect, onQuery, onFilter }: { workflow: Workflow; state: WorkspaceState; selectedId: string; query: string; filter: LocationState["filter"]; onSelect: (id: string) => void; onQuery: (query: string) => void; onFilter: (filter: LocationState["filter"]) => void }) {
...
function ResultDetail({ item }: { item: DashboardRequest | EvaluationRequest }) {
  ...
  return <div className="detail-stack"><div className="result-hero"><div className="score-result"><span>최종 안전운전 점수</span><strong>{item.result.score}</strong><small>/ 100점</small></div><div className="discount-result"><span>해당 할인 구간</span><strong>{outcome.eligible ? `${outcome.discountPercent}%` : "미충족"}</strong><small>{outcome.eligible ? "보험료 할인" : "조건 미달"}</small></div></div><dl className="result-grid"><div><dt>평가기간</dt><dd>{item.result.evaluationPeriod}</dd></div><div><dt>누적 주행거리</dt><dd>{item.result.accumulatedDistanceKm} km</dd></div><div><dt>적용 규칙</dt><dd>{item.rule.version}</dd></div><div><dt>최소 조건</dt><dd>{item.rule.minimumScore}점 · {item.rule.minimumDistanceKm} km</dd></div></dl><div className="privacy-note"><span className="privacy-icon">⌾</span><div><strong>원본 주행기록은 제공되지 않았습니다</strong><p>정확한 위치, 이동경로, 운행시각과 구간별 속도 없이 승인된 계산 결과만 표시됩니다.</p></div></div></div>;
}

function ProofDetail({ item }: { item: DashboardRequest | EvaluationRequest }) {
  const valid = item.proofStatus === "valid";
  const rows = [["규칙 해시", "0x7ad4…91c2", "일치"], ["Dataset Root", "0x2fe8…3b19", "유효"], ["상태 커밋먼트", "0x984c…a721", "연결됨"], ["Nullifier", "0xa10d…77e4", "미사용"]];
  return <div className="detail-stack"><div className={`verification-card ${valid ? "valid" : "invalid"}`}><span className="verification-icon">{valid ? <Check size={22} /> : <X size={22} />}</span><div><strong>{valid ? "Midnight 증명 검증 완료" : "Midnight 증명 검증 실패"}</strong><p>{valid ? "승인 규칙과 계산 입력, 이전·신규 상태 사이의 관계가 유효합니다." : "제출된 증명과 승인 규칙의 검증 값이 일치하지 않습니다."}</p></div></div><div className="proof-table">{rows.map(([label, value, status], index) => <div className="proof-row" key={label}><span>{label}</span><code>{value}</code><strong className={!valid && index === 0 ? "danger-text" : ""}>{!valid && index === 0 ? "불일치" : status}</strong></div>)}</div></div>;
}

function ActionBar({ workflow, item, onConfirm }: { workflow: Workflow; item: DashboardRequest | EvaluationRequest; onConfirm: (confirmation: NonNullable<Confirmation>) => void }) {
  if (item.status === "completed") {
    const result = workflow === "dashboard" ? (item as DashboardRequest).decision === "approved" ? "특약 요청을 승인했습니다." : "특약 요청을 반려했습니다." : (item as EvaluationRequest).decision === "applied" ? "보험료 할인을 적용했습니다." : "보험료 할인을 적용하지 않았습니다.";
  ...
  if (workflow === "dashboard") return <div className="action-bar"><div><strong>특약 요청을 승인할까요?</strong><span>{blocked ? "검증 실패 사유를 확인해 주세요." : "검증 결과를 확인한 뒤 승인해 주세요."}</span></div><div className="action-buttons"><button className="button secondary" type="button" disabled={blocked} onClick={() => onConfirm({ workflow, requestId: item.id, decision: "rejected", title: "특약 요청을 반려할까요?", description: `${item.applicantName}님의 요청이 처리 완료로 이동합니다.` })}>요청 반려</button><button className="button primary" type="button" disabled={blocked} onClick={() => onConfirm({ workflow, requestId: item.id, decision: "approved", title: "특약 요청을 승인할까요?", description: `${item.applicantName}님의 특약 가입 요청을 승인합니다.` })}>특약 승인</button></div></div>;
  const evaluation = item as EvaluationRequest;
  const outcome = getEvaluationOutcome(evaluation);
  return <div className="action-bar"><div><strong>할인 적용 여부를 결정해 주세요</strong><span>{outcome.eligible ? "결정 결과를 저장하고 처리 이력에 남깁니다." : "검증 결과와 할인 조건을 확인해 주세요."}</span></div><div className="action-buttons"><button className="button secondary" type="button" disabled={!outcome.eligible} onClick={() => onConfirm({ workflow, requestId: item.id, decision: "not-applied", title: "할인을 적용하지 않을까요?", description: `${item.applicantName}님의 평가 요청을 미적용으로 완료합니다.` })}>미적용</button><button className="button primary" type="button" disabled={!outcome.eligible} onClick={() => onConfirm({ workflow, requestId: item.id, decision: "applied", title: `${outcome.discountPercent}% 할인을 적용할까요?`, description: `${item.applicantName}님의 보험료 할인 결정을 저장합니다.` })}>할인 적용</button></div></div>;
}

function DetailPanel({ workflow, item, tab, onTab, onConfirm }: { workflow: Workflow; item: DashboardRequest | EvaluationRequest; tab: DetailTab; onTab: (tab: DetailTab) => void; onConfirm: (confirmation: NonNullable<Confirmation>) => void }) {
  const status = item.status === "completed" ? "처리 완료" : workflow === "dashboard" ? item.proofStatus === "invalid" ? "검토 필요" : "승인 대기" : item.proofStatus === "valid" ? "증명 유효" : "검증 무효";
  return <section className={`panel detail-panel tab-${tab}`}>
    <div className="detail-heading"><div><span className="detail-eyebrow">{workflow === "dashboard" ? "특약 요청 상세" : "평가 상세"}</span><h2>{item.applicantName} · {item.displayId}</h2><p>{item.productName} · {item.riderName}</p></div><StatusBadge tone={item.proofStatus === "invalid" ? "danger" : "success"}>{status}</StatusBadge></div>
...
export function App({ adapter = fixtureAdapter }: { adapter?: InsurerWorkspaceAdapter }) {
  ...
  const source = state ? location.workflow === "dashboard" ? state.requests : state.evaluations : [];
  const selected = useMemo(() => source.find((item) => item.id === location.requestId) ?? source[0], [source, location.requestId]);
  useEffect(() => { if (!selected) return; const next = { ...location, requestId: selected.id }; const target = toUrl(next); if (`${window.location.pathname}${window.location.search}` !== target) window.history.replaceState(null, "", target); }, [location, selected]);
  ...
  return <><div className="app-shell" aria-hidden={confirmation || utility ? true : undefined}><SideNav active={location.workflow} onNavigate={(workflow) => navigate(workflow)} onUtility={setUtility} /><main className="main-content"><PageHeader workflow={location.workflow} onUtility={setUtility} />{!state ? <section className="panel async-state">{loadError ? <><CircleAlert size={24} /><strong>요청을 불러오지 못했습니다.</strong><span>잠시 후 다시 시도해 주세요.</span><button className="button primary" type="button" onClick={() => setLoadAttempt((value) => value + 1)}>다시 불러오기</button></> : <><span className="loading-dot" /><strong>업무 요청을 불러오는 중입니다.</strong></>}</section> : <><SummaryGrid workflow={location.workflow} state={state} />{location.workflow === "dashboard" ? <div className="workflow-banner"><div><strong>승인 업무는 대시보드에서 처리해요</strong><p>특약 관리에서는 조건과 할인율을 확인하고, 승인 요청은 이 화면에서 한 번만 처리합니다.</p></div><button type="button" onClick={() => { const pending = state.requests.find((item) => item.status === "pending"); if (pending) navigate("dashboard", pending.id); }}>대기 요청 처리 →</button></div> : null}<div className="workspace-grid"><RequestList key={location.workflow} workflow={location.workflow} state={state} selectedId={selected?.id ?? ""} query={location.query} filter={location.filter} onQuery={(query) => updateLocation({ ...location, query }, true)} onFilter={(filter) => updateLocation({ ...location, filter })} onSelect={(id) => navigate(location.workflow, id, location.tab)} />{selected ? <DetailPanel workflow={location.workflow} item={selected} tab={location.tab} onTab={(tab) => navigate(location.workflow, selected.id, tab)} onConfirm={openConfirmation} /> : <section className="panel empty-detail"><strong>선택할 요청이 없습니다.</strong></section>}</div></>}</main></div>{confirmation ? <ConfirmDialog confirmation={confirmation} busy={saving} error={saveError} onCancel={() => setConfirmation(null)} onApply={applyConfirmedDecision} /> : null}{utility ? <UtilityDialog name={utility} onClose={() => setUtility(null)} /> : null}</>;
}

function UtilityDialog({ name, onClose }: { name: string; onClose: () => void }) {
  const ref = useRef<HTMLDialogElement>(null);
  useEffect(() => { ref.current?.showModal(); }, []);
  const content: Record<string, string> = { "도움말": "이 화면은 디자인 검토용 데모입니다. 표시된 가입자·통계·증명 값은 예시이며, 결정은 이 브라우저에만 저장됩니다. 실제 보험 API와 Midnight 네트워크는 연결되지 않았습니다.", "특약 관리": "안전운전 할인특약 · SAFE-DRIVE v1.4 · 누적 500 km 이상, 80점 이상 10%, 90점 이상 12%. 요청 승인과 반려는 대시보드에서 처리합니다.", "규칙 변환": "규칙은 담당자가 수기로 입력하고 검토·승인하는 경로를 사용합니다. 이 데모에는 규칙 편집·문서 변환 기능이 연결되지 않았습니다.", "검증 이력": "대시보드 또는 평가 요청에서 가입자를 선택한 뒤 ‘검증 결과 / 증명 검증’과 ‘처리 이력’ 탭을 확인하세요.", "연동 설정": "현재 브라우저에 저장되는 독립 데모입니다. 보험사 시스템·가입자 앱·Midnight 네트워크 연결은 설정되지 않았습니다.", "알림": "새로운 알림이 없습니다.", "계정": "미래손해보험 · 상품운영팀 · 데모 담당자" };
