...
import { Bell, Check, CircleAlert, CircleDot, CircleHelp, FileCheck2, Search, X } from "lucide-react";
...
function DetailPanel({ workflow, item, tab, onTab, onConfirm }: { workflow: Workflow; item: DashboardRequest | EvaluationRequest; tab: DetailTab; onTab: (tab: DetailTab) => void; onConfirm: (confirmation: NonNullable<Confirmation>) => void }) {
  ...
  const status = item.status === "completed" ? completedStatus : workflow === "dashboard" ? item.proofStatus === "invalid" ? "검토 필요" : "승인 대기" : item.proofStatus === "valid" ? "증명 유효" : "검증 무효";
