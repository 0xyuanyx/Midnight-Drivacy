...
describe("simulated driving generator",()=>{
 ...
 it("keeps tiny minimum distances schema-valid without padding the total",()=>{for(const minimumDistanceM of [1,2,3,4,5]){const first=generateTrip({...rule,minimumDistanceM},0,fixed);const distance=first.records.reduce((sum,record)=>sum+record.distanceM,0);expect(distance).toBeGreaterThan(0);expect(first.records.every(record=>record.distanceM>0)).toBe(true);expect(TripSchema.parse(first)).toEqual(first);}});
 it("uses independent low-frequency events without guaranteeing a score",()=>{const none=generateTrip(rule,0,{next:()=>0.5});const events=generateTrip(rule,0,{next:()=>0.05});expect(none.records.every(record=>record.speedingCount+record.accelerationCount+record.brakingCount===0)).toBe(true);expect(events.records.every(record=>record.speedingCount+record.accelerationCount+record.brakingCount===3)).toBe(true);});
});
