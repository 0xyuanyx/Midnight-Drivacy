...
export class PgRuleRegistrationRepository implements RuleRegistrationRepository {
  ...
  public async withRegistrationLock<T>(scopeId: string, ruleVersionId: string, task: () => Promise<T>): Promise<T> { const c=await this.pool.connect();try{await c.query("BEGIN");
    // DB UNIQUE는 chain transaction 이후의 중복 INSERT만 막는다. C 호출 전 동일 작업을 직렬화해야 한다.
    await c.query("SELECT pg_advisory_xact_lock(hashtextextended($1, 0), hashtextextended($2, 0))",[scopeId,ruleVersionId]); const value=await task();await c.query("COMMIT");return value;
