...
export interface ChainScopeDeploymentRow { id: string; evaluationScopeId: string; adapterProfile: string; network: "fixture" | "local" | "preprod"; chainContractAddress: string; deploymentTransactionId: string; currentRuleVersionId: string | null; }
...
export interface RuleRegistrationRepository {
  ...
  createDeploymentAndRegistration(deployment: Omit<ChainScopeDeploymentRow, "id" | "currentRuleVersionId">, registration: Omit<RuleRegistrationRow, "id" | "chainScopeDeploymentId">): Promise<{ deployment: ChainScopeDeploymentRow; registration: RuleRegistrationRow }>;
...
export class PgRuleRegistrationRepository implements RuleRegistrationRepository {
  ...
  public async findDeployment(scopeId: string, runtime: AdapterRuntime): Promise<ChainScopeDeploymentRow | undefined> { const r=await this.pool.query<ChainScopeDeploymentRow>(`SELECT id,evaluation_scope_id AS "evaluationScopeId",adapter_profile AS "adapterProfile",network,chain_contract_address AS "chainContractAddress",deployment_transaction_id AS "deploymentTransactionId",current_rule_version_id AS "currentRuleVersionId" FROM public.chain_scope_deployments WHERE evaluation_scope_id=$1 AND network=$2 AND adapter_profile=$3`,[scopeId,runtime.network,runtime.adapterProfile]);return r.rows[0]; }
  ...
  public async createDeploymentAndRegistration(d: Omit<ChainScopeDeploymentRow,"id"|"currentRuleVersionId">, x: Omit<RuleRegistrationRow,"id"|"chainScopeDeploymentId">) { const c=await this.pool.connect();try{await c.query("BEGIN");const deployment=await this.insertDeployment(c,d);const registration=await this.insertRegistration(c,{...x,chainScopeDeploymentId:deployment.id});
    // 승인만으로는 체인 적용본이 아니다. 등록 이력 저장과 current 변경을 함께 확정한다.
    const updated=await c.query<ChainScopeDeploymentRow>(`UPDATE public.chain_scope_deployments SET current_rule_version_id=$2 WHERE id=$1 RETURNING id,evaluation_scope_id AS "evaluationScopeId",adapter_profile AS "adapterProfile",network,chain_contract_address AS "chainContractAddress",deployment_transaction_id AS "deploymentTransactionId",current_rule_version_id AS "currentRuleVersionId"`,[deployment.id,x.ruleVersionId]);await c.query("COMMIT");return{deployment:updated.rows[0],registration};}catch(e){await c.query("ROLLBACK");throw e;}finally{c.release();} }
  public async createRegistrationAndSetCurrent(x: Omit<RuleRegistrationRow,"id">) { const c=await this.pool.connect();try{await c.query("BEGIN");const registration=await this.insertRegistration(c,x);
    // 과거 registration 재조회는 여기로 오지 않으므로 current를 되돌리는 rollback이 발생하지 않는다.
    await c.query(`UPDATE public.chain_scope_deployments SET current_rule_version_id=$2 WHERE id=$1`,[x.chainScopeDeploymentId,x.ruleVersionId]);await c.query("COMMIT");return registration;}catch(e){await c.query("ROLLBACK");throw e;}finally{c.release();} }
}
