...
// 모의 기록의 독립 이벤트 빈도다. 할인 성공을 보장하지 않으며 실제 Core가 점수를 결정한다.
const EVENT_PROBABILITY = 0.08;
...
export const generateTrip = (rule: Rule, previousConfirmedDistanceM: number, random: RandomProvider = systemRandom): Trip => {
  ...
  const target = rule.minimumDistanceM > 0 && previousConfirmedDistanceM === 0
    ? Math.max(1, Math.floor(rule.minimumDistanceM * ratio))
  ...
  const count = Math.min(target, 3 + Math.floor(random.next() * 6));
  ...
  const records = distances.map((distanceM, index) => ({ index, distanceM, durationSeconds: Math.max(1, Math.round(distanceM / SPEED_M_PER_SECOND)), speedingCount: Number(random.next() < EVENT_PROBABILITY), accelerationCount: Number(random.next() < EVENT_PROBABILITY), brakingCount: Number(random.next() < EVENT_PROBABILITY) }));
